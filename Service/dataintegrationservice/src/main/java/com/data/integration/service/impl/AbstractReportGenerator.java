...
/**
 * This is the base class used with the report generation examples. It contains
 * the actual <code>embedding</code> of the reporting engine and report
 * generation. All example embedded implementations will need to extend this
 * class and perform the following:
 * <ol>
 * <li>Implement the <code>getReportDefinition()</code> method and return the
 * report definition (how the report definition is generated is up to the
 * implementing class).
 * <li>Implement the <code>getTableDataFactory()</code> method and return the
 * data factory to be used (how this is created is up to the implementing
 * class).
 * <li>Implement the <code>getReportParameters()</code> method and return the
 * set of report parameters to be used. If no report parameters are required,
 * then this method can simply return <code>null</code>
 * </ol>
 */
public abstract class AbstractReportGenerator {
	 ...
	 * returns <code>null</code>, the report generation process will use the
	 * data factory used in the report definition.
	 *
	 ...
	 *             indicates an error generating the report
	 */
	public void generateReport(final OutputType outputType,
			OutputStream outputStream) throws IllegalArgumentException,
			ReportProcessingException {
		if (outputStream == null) {
			throw new IllegalArgumentException(
					"The output stream was not specified");
		}
